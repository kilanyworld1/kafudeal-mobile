...
import { View, Text, ScrollView, Pressable, Image, StyleSheet, Linking, I18nManager } from "react-native";
...
export default function Account() {
  ...
  const PRIMARY_MENU: MenuItem[] = [
    {
      ...
      label: t("account.saved_deals"),
      sub: `${saved.length} ${saved.length === 1 ? t("account.favorite") : t("account.favorites")}`,
      route: "/saved",
    },
    {
      icon: "cube", iconBg: "#FFE7D1", iconColor: "#FF6B2C",
      label: t("account.your_orders"),
      route: "/(tabs)/orders",
    },
    {
      icon: "ticket", iconBg: "#DBEAFE", iconColor: "#1D4ED8",
      label: t("account.vouchers"),
      sub: t("account.vouchers_active", { count: 2 }),
      route: "/vouchers",
    },
    {
      icon: "location", iconBg: "#ECFDF5", iconColor: "#15803D",
      label: t("account.saved_addresses"),
      sub: t("account.saved_count", { count: addressCount }),
      route: "/addresses",
    },
  ];
  ...
  // chevron-forward should visually point in the reading direction.
  // In RTL we mirror it so it points left (toward the row's content end).
      ...
      >
        <View style={s.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.country}>
              {isLoggedIn
                ? (user?.email || t("account.country_uae"))
        ...
        {!isLoggedIn && (
          <Pressable onPress={() => router.push("/login")} style={s.signinCard}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.signinTitle}>{t("auth.sign_in_to_kafudeal")}</Text>
              <Text style={s.signinSub}>{t("auth.sign_in_sub")}</Text>
            </View>
        ...
        <Pressable onPress={openIG} style={s.igCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.igTitle}>{t("account.follow_ig")}</Text>
            <Text style={s.igSub}>{t("account.follow_ig_sub")}</Text>
          </View>
        ...
        <Text style={s.versionText}>{t("account.version")}</Text>
      </ScrollView>
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  avatar: {
    width: 56, height: 56, borderRadius: 28, backgroundColor: "#FF6B2C",
  ...
  signinCard: {
    marginHorizontal: 16, marginTop: 16, backgroundColor: "white", borderRadius: 16, padding: 16,
    flexDirection: "row", alignItems: "center", gap: 12,
    shadowColor: "#0F172A", shadowOpacity: 0.06,
    shadowOffset: { width: 0, height: 4 }, shadowRadius: 12,
  },
  signinIcon: {
    width: 44, height: 44, borderRadius: 22, backgroundColor: "#FFE7D1",
    alignItems: "center", justifyContent: "center",
  },
  signinTitle: { fontSize: 15, fontWeight: "800", color: "#0F172A" },
  signinSub: { fontSize: 11.5, color: "#64748B", marginTop: 2 },
  signinBtn: { backgroundColor: "#FF6B2C", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999 },
  signinBtnText: { color: "white", fontSize: 12, fontWeight: "800" },
  menuGroup: { marginHorizontal: 16, marginTop: 14, backgroundColor: "white", borderRadius: 16, overflow: "hidden" },
  menuItem: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14 },
  menuItemBorder: { borderBottomWidth: 0.5, borderBottomColor: "rgba(15,23,42,0.06)" },
  menuIcon: { width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center" },
  menuLabel: { flex: 1, fontSize: 14, fontWeight: "700", color: "#0F172A" },
  menuMeta: { fontSize: 11.5, color: "#94A3B8", marginEnd: 6 },
  igCard: {
    marginHorizontal: 16, marginTop: 18, backgroundColor: "white", borderRadius: 14,
  ...
  signOutBtn: {
    marginHorizontal: 16, marginTop: 18, backgroundColor: "white",
